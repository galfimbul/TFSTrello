...
import ru.shvetsov.myTrello.dataClasses.TrelloConstants.SINGLE_BOARD_VIEW_MODEL_POS
...
class SingleBoardFragmentRepository @Inject constructor(val retrofit: SingleBoardApi, val token: String) {
    ...
    fun addCard(name: String, pos: String, idList: String): Single<Card> {
        return retrofit.addCard(name, pos, idList, CONSUMER_KEY, token)
    }
    ...
    fun moveColumn(oldColumnId: String, newColumnPos: Float): Single<Column> {
        return retrofit.moveColumn(oldColumnId, newColumnPos, CONSUMER_KEY, token)
    }

    fun addColumnToArchive(columnId: String): Single<Column> {
        return retrofit.addColumnToArchive(columnId, true, CONSUMER_KEY, token)
    }
    ...
    fun changeColumnName(newName: String, columnId: String): Single<Column> {
        return retrofit.changeColumnName(columnId, newName, CONSUMER_KEY, token)
    }
